  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-border bg-background/80 backdrop-blur-md">
      <div className="container mx-auto flex h-16 max-w-7xl items-center justify-between px-4 animate-fade-in-up">
        ...
        <button
          className="h-10 w-10 p-3 md:hidden"
          onClick={function () {
            ...
            {navItems.map(function (item) {
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className="min-h-10 flex items-center text-sm font-medium text-muted-foreground transition-all duration-200 hover:text-foreground px-2 py-2"
                  onClick={function () {
                    setMobileOpen(false);
                  }}
                >
